    ...
    const partTwo = (data: string) => {
        const lines = data.split("\n")
            .map((line, index) => {
                ...
                const id = index + 1;
                const numbersArrays = numbersPart
                .split("|")
                .map((numbers) =>
                    numbers.trim().split(/\s+/).map((numStr) => parseInt(numStr, 10))
                );
                
                return { id, numbersArrays };
            });

        for (let i = 0; i < lines.length; i++) {
            const commonNumbers = lines[i].numbersArrays[0].filter((number) => lines[i].numbersArrays[1].includes(number));
            // console.log(commonNumbers)
        
            // Clone the next lines based on the number of common numbers
            for (let j = 1; j <= commonNumbers.length; j++) {
                if (i + j < lines.length) {
                    const lineId = lines[i].id;
                    console.log(lineId);
                    const newId = lines[i + j].id
                    // console.log('id: ', lineId);
                    // console.log('new id: ', newId);
                    const newNumbersArrays = [...lines[i + j].numbersArrays];
                    // console.log('newNumbersArrays: ', newNumbersArrays);
                    lines.push({ id: newId, numbersArrays: newNumbersArrays });
