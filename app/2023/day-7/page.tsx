...
const TwentyTwentyThree_DaySeven = () => {
  ...
  const partOne = (data: string) => {
    ...
    const combinedData = allHands.concat(
      fiveOfAKind,
      fourOfAKind,
      fullHouse,
      threeOfAKind,
  ...
  const partTwo = (data: string) => {
    ...
    const isTwoPair = (cardLabels: string) => {
      ...
      return (
        pairCounts.length >= 2 ||
        (pairCounts.length >= 1 && hasJ && jCount === 1) ||
        (pairCounts.length >= 0 && hasJ && jCount === 2)
      );
    };
    ...
    const isOnePair = (cardLabels: string) => {
      ...
      return (
        pairCounts.length >= 1 ||
        (pairCounts.length >= 0 && hasJ && jCount === 1)
      );
    };
